...
public class DocxToPdfUtil {
    ...
    /**
     * DOCX 파일이 있을 경우 → PDF 경로 반환
     * 이미 PDF가 있으면 그걸 쓰고, 없으면 변환
     */
    public static File getOrConvertPdfFromDocx(File docxFile) throws IOException, InterruptedException {
        String baseName = docxFile.getName().replaceAll("\\.docx$", "").replaceAll("\\s+", "_");
     ...
     */
    private static void convertDocxToPdf(File docxFile, File outputPdf) throws IOException, InterruptedException {
